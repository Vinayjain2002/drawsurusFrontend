...
export default function KeywordUpload() {
  ...
  const handleAddWord = async () => {
    ...
    try {
      ...
      dispatch(addCustomWord(wordObj))
      setNewWord("")
      setError(null)
    } catch (err) {
  ...
  const handleRemoveWord = useCallback(
    async (wordToRemove: string) => {
      setIsLoading(true)
      try {
        // Assuming the endpoint supports deleting words; if not, skip this API call
        // and only update Redux state, or implement a separate delete endpoint
        ...
        <div>
          ...
          {customWords.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {customWords.map((wordObj) => (
                <Badge key={`${wordObj.word}-${wordObj.roomID}`} className="gap-1.5">
                  {wordObj.word} ({wordObj.difficulty}, Room: {wordObj.roomID})
                  <Button
                    variant="ghost"
                    size="sm"
